#ifndef SNIPER_AI_ULTRA_SHELL_B_MQH
#define SNIPER_AI_ULTRA_SHELL_B_MQH
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   Print("SNIPER AI ULTRA ULTIMATE-MODULAR Loaded BUILD_ID=SA_ULTRA_94 MaxOpen=", MaxOpenTrades);
   UltraCoreInit();
   if(EnableAPEXStrategy || EnableContFallback || EnableLCSStrategy)
      Print("OK94 WARNING: old APEX/ContFallback/LCS input ON — evaluators STUBBED; ULTRA only fires");
   Print("OK94 ULTIMATE STRUCTURE: modules 00-30 loaded (Types→Recovery) — ULTRA only");
...
{
   // OK94 REMOVED — ContFallback deleted (ULTRA ContSniper replaces it)
   detail = "ContFallback retired OK94";
   return false;
}
...
bool PRIME_IsLiveTag(const string tag); // OK94 forward — Ultra live tags
void UltraCoreInit();
...
{
   // OK94 REMOVED — old ContSniper/Rev/Instant/PRISM live router
...
{
   // OK94: IDP retired as live gate — ULTRA confluence owns decisions
   detail = "IDP retired soft-pass OK94";
   return true;
}
...
{
   // OK94 REMOVED — old APEX live strategy deleted
   detail = "APEX retired OK94";
   invalidation = 0.0;
   return false;
}
...
{
   // OK94 REMOVED — never opens trades
   buySignal = false; sellSignal = false; strategyTag = "";
}
...
{
   // OK94 REMOVED — LCS deleted from live path
   buySignal = false; sellSignal = false; strategyTag = "";
}
...
void AnalyzeLiveMarket(const bool force)
{
   ...
   m.contBuyDetail = "";
   m.contSellDetail = "";
   // OK94: live analysis from ULTRA only (old ContFallback/APEX probes removed)
   {
      UltraSnap us;
...
{
   AnalyzeLiveMarket(true);
   Print("---- MARKET ANALYSIS BUILD=SA_ULTRA_94 (", BrokerSymbol, ") ----");
...
void InstantExecution()
{
   ...
   // EvaluateStrategySignals() = OK94 SNIPER AI ULTRA complete architecture.
